...
describe('apply.recheck', function()
  ...
  it('leaves a buffer with unsaved edits alone and names it', function()
    ...
    local reloaded, left = apply.recheck(root, { run_id = 'r1', nofade = true })
    eq({}, reloaded)
    eq({ path }, { left[1] and left[1].path })
    ok(left[1].reason:find('unsaved edits', 1, true) ~= nil, 'got: ' .. tostring(left[1].reason))
    eq({ 'one', 'mine' }, lines_of(buf), 'the hand edit survives')
    cleanup()
  end)

  it('names a file the shell step deleted instead of dropping it', function()
    local path, buf = open_file('hello\n')
    local root = vim.fs.dirname(path)
    -- `rm`, `git clean`, a build script: the buffer holds a file that is gone.
    eq(0, vim.fn.delete(path))

    local reloaded, left = apply.recheck(root, { run_id = 'r1', nofade = true })
    eq({}, reloaded)
    eq(1, #left, 'a delete is a change to the file, and must not be silent')
    eq(path, left[1].path)
    ok(left[1].reason:find('gone from disk', 1, true) ~= nil, 'got: ' .. tostring(left[1].reason))
    eq({ 'hello' }, lines_of(buf), 'the content is left in the buffer for the user to save or discard')
    vim.cmd('silent! bwipeout! ' .. buf)
    cleanup()
  end)

  it('does not call a buffer over a file that never existed a deletion', function()
    local path = open_file('one\n')
    local root = vim.fs.dirname(path)
    vim.cmd('edit ' .. vim.fn.fnameescape(root .. '/not-written-yet.py'))
    local fresh_buf = vim.api.nvim_get_current_buf()
    local reloaded, left = apply.recheck(root, { run_id = 'r1', nofade = true })
    eq({}, reloaded)
    eq({}, left, '`:e newfile` is not a file a shell step removed')
    vim.cmd('silent! bwipeout! ' .. fresh_buf)
    cleanup()
  end)
