...
describe('config.setup', function()
  ...
  it('opens chat fresh by default, and lets resume-last be chosen instead', function()
    eq('new', config.setup(nil).chat.default)
    eq('resume-last', config.setup({ chat = { default = 'resume-last' } }).chat.default)
    t.throws(function()
      config.setup({ chat = { default = 'always-resume' } })
    end, 'chat.default')
    config.setup(nil)
  end)
end)
...
describe('config debug level', function()
  it('defaults the debug log to off', function()
    local opts = config.setup(nil)
    eq('off', opts.debug.level)
  end)

  it('accepts the levels it documents and rejects anything else', function()
    eq('debug', config.setup({ debug = { level = 'debug' } }).debug.level)
    eq('info', config.setup({ debug = { level = 'info' } }).debug.level)
    t.throws(function()
      config.setup({ debug = { level = 'verbose' } })
    end, 'debug.level')
    t.throws(function()
      config.setup({ debug = { levl = 'info' } })
    end, 'unknown option debug.levl')
    config.setup()
  end)
end)
