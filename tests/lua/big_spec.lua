...
describe('big change session states', function()
  ...
  it('lists the project big changes with their state in the picker', function()
    ...
    ok(lines[1]:match('start a new change') ~= nil, 'the picker leads with the escape to a fresh change')
    ok(lines[2]:match('building') ~= nil, lines[2])
    ok(lines[3]:match('mergeable') ~= nil, lines[3])
    vim.api.nvim_win_close(win, true)
    cleanup()
  end)

  it('opens a fresh change from the picker instead of only listing existing ones', function()
    local _, path = sandbox()
    open_on(path)
    local live = big.state()
    live.session = session({ id = 'old', conversation = { { role = 'user', text = 'old request' } } })
    fake.replies['big.list'] = { result = { sessions = { session({ id = 'old', title = 'old change' }) } } }
    big.pick_session()
    vim.api.nvim_feedkeys(vim.api.nvim_replace_termcodes('<CR>', true, true, true), 'x', false)
    eq(nil, live.session, 'the picker´s first row started a clean change')
    eq(nil, sent('big.open'), 'picking "start a new change" never re-opens an existing session')
    ok(has_line('new big change'), 'the panel shows the fresh-start state')
    cleanup()
  end)

  it('opens the picker with no existing changes instead of refusing', function()
    local _, path = sandbox()
    open_on(path)
    local before = #vim.api.nvim_list_wins()
    fake.replies['big.list'] = { result = { sessions = {} } }
    big.pick_session()
    ok(#vim.api.nvim_list_wins() > before, '"start a new change" makes the picker useful even with zero sessions')
    local win = vim.fn.win_getid(vim.fn.winnr())
    local lines = vim.api.nvim_buf_get_lines(vim.api.nvim_win_get_buf(win), 0, -1, false)
    ok(lines[1]:match('start a new change') ~= nil, lines[1])
    vim.api.nvim_win_close(win, true)
    cleanup()
  end)
