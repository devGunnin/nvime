...
# Bystanders outlive their stub by design (nothing may kill them), so they are
# this script's to clean up.
# shellcheck disable=SC2329  # invoked from the EXIT trap below
cleanup() {
  local path pid root
  for path in "$lab"/*.bystander.*.pid; do
    [ -f "$path" ] || continue
    pid="$(cat "$path")"
...
# ---- 3. a record without a live claim is never signalled -------------------
...
# ---- 4. a reaper that cannot read the store is never a pass ----------------
...
# ---- 5. names, deadlines, and the mirrored staleness rule ------------------
